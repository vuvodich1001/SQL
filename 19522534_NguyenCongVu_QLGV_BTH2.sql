...
--CAU 5
SELECT HV.MAHV, HV.HO, HV.TEN FROM HOCVIEN HV, KETQUATHI KQ 
WHERE LEFT(KQ.MAHV, 1) = 'K' AND KQ.MAMH = 'CTRR' AND KQ.LANTHI = '3' 
AND KQ.KETQUA = 'Khong Dat' AND HV.MAHV = KQ.MAHV
...
-- CAU 17 KHO QUA
SELECT HV.HO, HV.TEN, DIEM FROM HOCVIEN HV INNER JOIN KETQUATHI K ON HV.MAHV = K.MAHV	
WHERE K.MAMH = 'CSDL'
GROUP BY HV.HO, HV.TEN, DIEM
HAVING COUNT(*) = MAX(K.LANTHI) 
ORDER BY HV.TEN ASC

SELECT HV.HO, HV.TEN, K.DIEM FROM HOCVIEN HV INNER JOIN KETQUATHI K ON HV.MAHV = K.MAHV
WHERE HV.MAHV IN (SELECT MAHV FROM KETQUATHI K WHERE K.LANTHI = (SELECT MAX(K.LANTHI) FROM KETQUATHI))
